...
#
# A tool is not optional here once any test file skips itself when that tool is absent. A
# Python-only runner passes green while testing none of the JavaScript, and that is worse than no
# CI — it looks like coverage. `tests/test_ci_does_not_report_success_on_a_skipped_suite.py` fails
# loudly if that ever regresses, so this file and that one have to stay in step: a tool in its
# list is installed here, and a tool installed here is in its list.
#
# A new repo skips on nothing, so both start empty. Uncomment a step below in the same commit that
# adds its tool to that list.
name: tests
